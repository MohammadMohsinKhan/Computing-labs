...
import math
def extractGrade(x):
    grade_percent = list(x.split('-')[1])
    grade_percent.remove('%')
    return ''.join(grade_percent)


def classAverage(final_marks):
    total = 0
    #totals the marks of the students
    for mark in final_marks:
        total = total + float(extractGrade(mark))
    #calculates the average of the students from the total
    average = total/(len(final_marks))
    return average

def classStdDev(final_marks):
    standard_deviation = 0.0
    average = classAverage(final_marks)
    #calculates standard deviation
    for mark in final_marks:
        standard_deviation += float((float(extractGrade(mark))-average)**2) 
    standard_deviation = math.sqrt(standard_deviation/len(final_marks))
...
# ```
# 1. no since the list with the student number and final grade percent is the only input the functions take.
# 2. the seperator in .split would be changed from - to :
# 3. no because we need the average to compute the standard deviation.
# ```
