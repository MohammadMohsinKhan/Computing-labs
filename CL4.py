...
#checks if the conditions are met for a 'GX' product
def passedTestsGX(testInfoGX):
    if getTestScores(testInfoGX)[0] >= 0.8: #checks if the T1 criteria is satisfied
        #checks if the T2 criteria is satisfied
        if getTestScores(testInfoGX)[1] >= 0.9:
            return True
        else:
            return False
    else:
...
#checks if the conditions are met for a 'BX' product
def passedTestsBX(testInfoBX):
    if getTestScores(testInfoBX)[0] >= 0.7: # checks if the T1 criteria is satisfied
        #checks if the T2 criteria is satisfied
        if getTestScores(testInfoBX)[1] >= 0.98 and getTestMonths(testInfoBX)[1] <=2:
            return True
...
def determinePassedProducts(products):
    ...
    for product in products:
        #checks wether the product is a 'BX' or 'GX' and then checks if it satisfies the conditions
        if getProductVersion(product) == "BX":
            if passedTestsBX(product):
                passed_products.append(product[0])
        elif getProductVersion(product) == 'GX':
            if passedTestsGX(product):
                passed_products.append(product[0])
...
# ```
# 1.no it can also be done with a while loop but its is beneficial to use a for loop as it is easier to use in this situation. an if condition will have to be setup to check how many tests the product has undergone.
# 2.yes it is possible for us to setup the determinePassedProducts function with a while loop but it will require more work when compared to using a foor loop. a while loop would be useful if the incriment value is nonstandard i.e. changing according to some conditions
# ```
