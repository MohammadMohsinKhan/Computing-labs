...
def extract_temps(temps):
    temps_extracted = [] #empty list where the extracted temperature values will be appended    
    
    for temp in temps: #loop runs through every value in the temps list and appends the integer and float values to the temps_extracted list in the float form
        try:
            temps_extracted.append(float(temp))
        except ValueError: #skips the element of the list if it isnt a float or integer value
            pass
    

    return temps_extracted

def selection_sort(nums):
    try:
        new_arr = nums.copy() #creates a copy of the nums list
        n = len(new_arr)
        
        for i in range(n): #for loop which sorts the list using selection sort algorithm
            min_index = i
            j = i
        ...
        return new_arr
    except: #raises an exception if the input isnt a list
        return "Input should be a list with integer or float values"

def calculate_median(nums):
    try:
        n = len(nums)
        if n%2 == 0: #checks if the list has even or odd number of values and then calculates the median accordingly
            median = float((nums[n//2] + nums[(n//2)-1])/2)
        else:
            median = float(nums[n//2])
    except ValueError: #exception statement to catch if the input for the function isnt a list or if the list doesnt have integer or float values
        return "Input should be a list with integer or float values"
    except TypeError: #exception statement to catch if the input for the function isnt a list or if the list doesnt have integer or float values
        return "Input should be a list with integer or float values"

    return median

def main(temp_data): #main function which uses all of the functions defined perviously to extract the median temperature
    try:
        return float(calculate_median(selection_sort(extract_temps(temp_data)))) 
    except:
        return "N/A"
